...
function doMatch() {
    ...
    billData.forEach((billRow, billIdx) => {
        ...
        checkinData.forEach((checkinRow, checkinIdx) => {
            if (matchedCheckinIdx.has(checkinIdx)) return;
        ...
        if (bestMatch) matchedCheckinIdx.add(bestMatchIdx);
...
function displayResults() {
    ...
    matchResults.forEach(result => {
        ...
        bodyHtml += '<td><span class="status-badge ' + result.status + '">' + (result.status === 'matched' ? '匹配' : '无登记') + '</span></td>';
...
function exportExcel() {
    ...
    matchResults.forEach((result, rowIdx) => {
        ...
        ws[XLSX.utils.encode_cell({r, c: c++})] = {v: result.status === 'matched' ? '匹配' : '无登记', t: 's'};
